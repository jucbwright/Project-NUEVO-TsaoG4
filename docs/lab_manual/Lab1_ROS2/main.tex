...
Wait until the log shows \texttt{[bridge] Starting bridge node...} followed by the bridge
startup messages.
The bridge node starts automatically --- you do not need to start it manually.

\textbf{Important:} never run \texttt{ros2 run bridge bridge} manually inside the container.
The container already runs exactly one bridge process.
Starting a second one causes conflicting publishers and a non-functional UI.

\subsubsection*{3.4 Verify the Bridge is Running}

Confirm only one bridge node is registered:
...
You should see exactly \texttt{/bridge} (and no duplicates).
