"""
zombie_detector.py - detect a "zombie" (a person that is green)
===============================================================
...
Then copy this file over main.py and restart the robot node:
    cp examples/zombie_detector.py main.py
...
def find_zombie(robot: Robot) -> dict | None:
    """
    Return the highest-confidence detection that is BOTH:
      - class "person"
      - color attribute "green"
    or None if no such target is visible.
    """
    ...
    # get_detections("person") returns only detections whose class is "person"
    for detection in robot.get_detections("person"):
        confidence = float(detection["confidence"])
        if confidence < MIN_CONFIDENCE:
            continue

        # Pull the color attribute the same way traffic_light_leds.py does
        attributes   = detection.get("attributes", {})
        color_attr   = attributes.get("color", {})
        color        = color_attr.get("value")          # e.g. "green", "red", None
    ...
    while True:
        ...
        # -- SCANNING -------------------------------------------------------
            ...
            if zombie is not None:
                ...
                # Timer expired — no zombie seen recently
